...
```ts showLineNumbers filename="src/mastra/index.ts" copy
const response = await myAgent.generate([{ role: 'user', content: 'Hello, how can you assist me today?' }]);
...
const response = await myAgent.generate(
  [
    {
      role: 'user',
      content: 'If a taxi driver earns $9461 per hour and works 12 hours a day, how much do they earn in one day?',
    },
  ],
  {
...
```ts showLineNumbers filename="src/mastra/agents/index.ts" copy
const response = await myAgent.generate([{ role: 'user', content: "Calculate the taxi driver's daily earnings." }], {
  maxSteps: 5,
...
```ts showLineNumbers filename="src/mastra/agents/index.ts" copy
const stream = await myAgent.stream([{ role: 'user', content: "Calculate the taxi driver's daily earnings." }], {
  maxSteps: 5,
  onFinish: ({
    steps,
    text,
