...
var DefaultExecutionEngine = class extends ExecutionEngine {
  ...
  async execute(params) {
    ...
    if (steps.length === 0) {
      throw new chunkU3XRKJNN_cjs.MastraError({
    ...
    for (let i = startIdx; i < steps.length; i++) {
      ...
      try {
        ...
        const error = e instanceof chunkU3XRKJNN_cjs.MastraError ? e : new chunkU3XRKJNN_cjs.MastraError(
          {
...
function createStep(params) {
  if (params instanceof chunk7KWHHSK5_cjs.Agent) {
    return {
      id: params.name,
      // @ts-ignore
